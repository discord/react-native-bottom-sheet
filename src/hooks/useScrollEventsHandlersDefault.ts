  ...
  useAnimatedReaction(
    ...
    contentOffsetY => {
      if (lockableScrollableContentOffsetY) {
        lockableScrollableContentOffsetY.value = contentOffsetY;
      }
    }
  );
    ...
    useWorkletCallback(
      (event, context) => {
        ...
        if (
          ...
          awaitingFirstScroll.value &&
          !isScrollableLocked.value
        ) {
          const isScrollingTowardsBottom =
            context.initialContentOffsetY < event.contentOffset.y;
          if (
            isScrollingTowardsBottom &&
            event.contentOffset.y > (scrollBuffer ?? 0) &&
            context.shouldLockInitialPosition
          ) {
